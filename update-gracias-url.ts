...
async function updateGraciasImage() {
	try {
		console.log(
			"DATABASE_URL:",
		...
		const minioUrl = `${process.env.S3_ENDPOINT}/${process.env.S3_BUCKET}/images/gracias.webp`;

		console.log("Updating 'Gracias' word image URL...");
		console.log("New URL:", minioUrl);

		// Update the word 'Gracias' with the new MinIO URL
		const result = await db
			.update(words)
			.set({ image: minioUrl })
			.where(eq(words.word, "Gracias"))
			.returning();
		...
		if (result.length > 0) {
			...
			console.log("\n⚠️ No word 'Gracias' found in database");
		}
